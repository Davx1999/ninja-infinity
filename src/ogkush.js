...
class OGInfinity {
  constructor() {
    ...
    this.universe = /s([0-9]+)-/.exec(window.location.pathname)[1];
    ...
    // OGame Ninja
    this.njaStore = `ogk-${this.universe}-${this.gameLang}-${document.querySelector("meta[name=ogame-player-id]").content}`;
    this.njaBotID = /\/bots\/([0-9]+)\//.exec(window.location.pathname)[1];

    console.log(`Storage: ${this.njaStore}`);
  }

  init() {
    let res = JSON.parse(localStorage.getItem(this.njaStore));
    this.json = res || {};
  ...
  start() {
    ...
    if (this.json.welcome) {
      if (this.page == "fleetdispatch") {
        ...
        window.location.href = `/bots/${this.njaBotID}/browser/html/s${this.universe}-${this.gameLang}?page=ingame&component=fleetdispatch`;
      }
    }
  ...
  fetchAndConvertRC(messageId) {
    const url = `/bots/${this.njaBotID}/browser/html/s${this.universe}-${this.gameLang}?page=messages&messageId=${messageId}&tabid=21&ajax=1`;
    return fetch(url)
      .then((rep) => rep.text())
      .then((str) => {
  ...
  generateHiscoreLink(playerid) {
    return `/bots/${this.njaBotID}/browser/html/s${this.universe}-${this.gameLang}?page=highscore&searchRelId=${playerid}`;
  }
  ...
  playerSearch(show, name) {
    let renderPlayerInfo = (player) => {
      ...
      let name = `<span>${player.name}</span> <span class="${this.getPlayerStatus(
        player.status
      )}"></span>\n                  <a target="_self"\n                    href="/bots/${this.njaBotID}/browser/html/s${this.universe}-${this.gameLang}?page=highscore&searchRelId=${
        player.id
  ...
  async getLifeformBonus() {
    ...
    return fetch(
      `/bots/${this.njaBotID}/browser/html/s${this.universe}-${this.gameLang}?page=ingame&component=lfbonuses`,
  ...
  async getEmpireInfo() {
    ...
    return fetch(
      `/bots/${this.njaBotID}/browser/html/s${this.universe}-${this.gameLang}?page=standalone&component=empire`,
      { signal: abortController.signal }
    )
      .then((rep) => rep.text())
      .then((str) => {
        ...
        if (hasMoon) {
          return fetch(
            `/bots/${this.njaBotID}/browser/html/s${this.universe}-${this.gameLang}?page=standalone&component=empire&planetType=1`,
            { signal: abortController.signal }
          )
            .then((rep) => rep.text())
            .then((str) => {
  ...
  updateresourceDetail() {
    ...
    for (const [coords, details] of Object.entries(flyingDetails)) {
      ...
      if (res > 0) {
        let href = `/bots/${this.njaBotID}/browser/html/s${this.universe}-${this.gameLang}?page=ingame&amp;component=galaxy&galaxy=${coords.split(":")[0]}&system=${
          coords.split(":")[1]
  ...
  sendMessage(id) {
    if (this.tchat) {
      ...
      document.location = `/bots/${this.njaBotID}/browser/html/s${this.universe}-${this.gameLang}?page=chat&playerId=${id}`;
    }
  }

  generateIgnoreLink(playerId) {
    return `/bots/${this.njaBotID}/browser/html/s${this.universe}-${this.gameLang}?page=ignorelist&action=1&id=${playerId}`;
  }

  generateBuddyLink(playerId) {
    return `/bots/${this.njaBotID}/browser/html/s${this.universe}-${this.gameLang}?page=ingame&component=buddies&action=7&id=${playerId}&ajax=1`;
  }
  ...
  deleteMSg(msgId) {
    ...
    return fetch(`/bots/${this.njaBotID}/browser/html/s${this.universe}-${this.gameLang}?page=messages`, {
      method: "POST",
      body: requestData,
    })
  ...
  saveData() {
    localStorage.setItem(this.njaStore, JSON.stringify(this.json));
  }
      ...
      close.addEventListener("click", () => {
        if (this.json.welcome) {
          ...
          if (this.playerClass == PLAYER_CLASS_NONE) {
            window.location.href = `/bots/${this.njaBotID}/browser/html/s${this.universe}-${this.gameLang}?page=ingame&component=characterclassselection`;
          } else {
            window.location.href = `/bots/${this.njaBotID}/browser/html/s${this.universe}-${this.gameLang}?page=ingame&component=overview`;
          }
        }
        overlay.classList.remove("ogl-active");
      });
